...
public class UsuarioController {
    ...
    @GetMapping
    public ResponseEntity <List<Usuario>> listar(){service.listarUsuario(); return ResponseEntity.ok().build();}

    @GetMapping("/{id}")
    public ResponseEntity<Usuario> buscarPorId(@PathVariable String id){service.buscarPorId(id); return ResponseEntity.ok().build();}
